...
class OptionService {
  ...
  async create(optionDto) {
    ...
    await this.alreadyExistByCtegoryAndKey(optionDto.key, optionDto.category);
  ...
  async update(id,optionDto) {
    ...
    if(optionDto.category && isValidObjectId(optionDto.category)){
        const category = await this.#categoryService.checkExistById(optionDto.category);
        optionDto.category = category._id;
    } else delete optionDto.category
    if(optionDto.slug){
        optionDto.key = slugify(optionDto.key, {
          trim: true,
          replacement: "_",
        ...
        await this.alreadyExistByCtegoryAndKey(optionDto.key, categoryid);
    }
    if (optionDto?.enum && typeof optionDto.enum == "string") {
      optionDto.enum = optionDto.enum.split(",");
  ...
  async alreadyExistByCtegoryAndKey(key, category) {
